...
while True:
    ...
    if packet == b"END":
        ...
        # ✅ STOP TIMER HERE (before hash)
        transfer_end_time = time.time()
        transfer_time = transfer_end_time - start_time

        print(f"\n--- Transfer Complete (Before Hash Check) ---")
        print(f"Time taken: {transfer_time:.2f} seconds")

        if transfer_time > 0:
            throughput = (bytes_received / (1024 * 1024)) / transfer_time
